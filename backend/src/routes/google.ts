...
import { exportPlan, fetchContent, getFile, listFolder, parseDriveUrl } from "../google/drive.ts";
...
googleRoutes.post("/auth/start", async (c) => {
	const { authUrl } = await oauth.startAuth();
	return c.json({ authUrl });
});

googleRoutes.get("/auth/status", async (c) => {
	return c.json(await oauth.authStatus());
});

googleRoutes.delete("/auth", async (c) => {
	await oauth.logout();
	return c.json({ ok: true });
});

googleRoutes.put("/client", async (c) => {
	...
	await oauth.setClientOverride(body.clientId.trim(), body.clientSecret?.trim() || undefined);
	return c.json({ ok: true });
});

googleRoutes.post("/drive/resolve", async (c) => {
	const { url } = (await c.req.json().catch(() => ({}))) as { url?: string };
	const parsed = parseDriveUrl(url ?? "");
	if (!parsed) throw badRequest("not a Drive URL");
	if (parsed.kind === "file") {
		const f = await getFile(parsed.id, parsed.resourceKey);
		return c.json({ kind: "file", files: [{ ...f, plan: exportPlan(f) }] });
	}
	const { files, folderName } = await listFolder(parsed.id);
	return c.json({
		kind: "folder",
		folderName,
...
googleRoutes.post("/drive/import", async (c) => {
	...
	(async () => {
		for (const item of items ?? []) {
			...
			try {
				const f = await getFile(item.driveId, item.resourceKey);
				const plan = exportPlan(f);
				if (plan.mode === "skip") {
					emit({ type: "error", driveId: item.driveId, message: plan.reason });
					continue;
				}
				const bytes = await fetchContent(f, item.resourceKey);
				...
				if (plan.card === "text") {
					emit({ type: "text", driveId: item.driveId, markdown: new TextDecoder().decode(bytes) });
				}
