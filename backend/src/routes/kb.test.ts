...
import { addChat, addFile, search, searchGraded } from "../kb/index.ts";
...
describe("KB routes", () => {
	...
	it("POST /api/kb/:canvas/files → 200 for HTML", async () => {
		const html =
			"<html><body><h1>Nyquist</h1><p>Sample at twice the highest frequency.</p></body></html>";
	...
	it("reranks the most relevant chunk to the top and grades it strong", async () => {
		...
		const { chunks, verdict } = await searchGraded(
			canvas,
			"Manhattan and Chessboard distance between pixels",
			3,
		);
	...
	it("grades verdict 'none' when the KB has no relevant content", async () => {
		...
		await addFile(
			...
			"text/plain",
			new TextEncoder().encode(
				"To bake sourdough, mix flour and water, ferment the starter, then proof and bake.",
			),
		);
