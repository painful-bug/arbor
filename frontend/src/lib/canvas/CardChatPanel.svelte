<script lang="ts">
    ...
    const node = $derived(
        flow.nodes.find((n) => n.id === flow.selected && n.type === "card"),
    );
...
<aside class="panel" class:open class:resizing style:width={open ? `${chatWidth}px` : "0px"}>
    <div class="inner" style:width="{chatWidth}px">
        ...
        {#if card && cardId}
            <header>
                <span class="dot" style="background: var(--block-{card.block})"
                ></span>
                <h3>{card.title}</h3>
            </header>
        ...
        {:else}
            ...
            <div class="body" bind:this={hubScroller}>
                {#if session.turns.length === 0}
                    <div class="hub-empty">
                        <p>
                            Ask anything about this canvas, or say <em
                                >"save as a card"</em
                            > to create one.
                        </p>
                    </div>
...
<style>
    ...
    /* disable spring during drag so resize tracks the pointer exactly */
    .panel.resizing {
        transition: none;
    }
    .inner {
