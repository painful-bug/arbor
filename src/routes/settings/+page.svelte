<script lang="ts">
	...
	// Load saved keys from Keychain on mount so fields show what's stored.
	$effect(() => {
		if (!isTauri()) return;
		(async () => {
			...
			for (const p of [...keyed.map((k) => k.id), 'tavily'] as string[]) {
				try {
					const k = await invoke<string | null>('keychain_get', { provider: p });
					if (k) {
						if (p === 'tavily') tavilyKey = k;
						else keys[p] = k;
					}
				} catch {
					/* no key stored yet */
	...
	async function saveKey(provider: string, value: string): Promise<boolean> {
		if (!isTauri()) return true; // browser dev: no keychain
		const { invoke } = await import('@tauri-apps/api/core');
		try {
			await invoke('keychain_set', { provider, key: value });
			return true;
		} catch {
			return false;
		}
