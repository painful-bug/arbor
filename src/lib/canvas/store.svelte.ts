...
// ── Storage helpers ──────────────────────────────────────────────────────────
// Reads/writes under ~/.loom via Tauri commands; falls back to localStorage in
// browser dev mode so SSR/vite-dev don't break.

function isTauri(): boolean {
	return typeof window !== 'undefined' && '__TAURI_INTERNALS__' in window;
}

async function storeRead(rel: string): Promise<string | null> {
	if (!isTauri()) {
		try { return localStorage.getItem(rel); } catch { return null; }
	}
	const { invoke } = await import('@tauri-apps/api/core');
	return invoke<string | null>('store_read', { rel });
}

function storeWrite(rel: string, contents: string): void {
	if (!isTauri()) {
		try { localStorage.setItem(rel, contents); } catch {}
		return;
	}
	import('@tauri-apps/api/core').then(({ invoke }) => {
		void invoke('store_write', { rel, contents });
	});
}

function storeDelete(rel: string): void {
	if (!isTauri()) {
		try { localStorage.removeItem(rel); } catch {}
		return;
	}
	import('@tauri-apps/api/core').then(({ invoke }) => {
		void invoke('store_delete', { rel });
	});
}
...
async function readIndex(): Promise<CanvasIndex | null> {
	const raw = await storeRead('canvases/index.json');
	if (!raw) return null;
	try { return JSON.parse(raw) as CanvasIndex; } catch { return null; }
}

function writeIndex(current: string, list: CanvasMeta[]): void {
	storeWrite('canvases/index.json', JSON.stringify({ current, list }));
}
...
export async function loadDoc(id: string): Promise<CanvasDoc | null> {
	const raw = await storeRead(`canvases/${id}.json`);
	if (!raw) return null;
	try {
...
function writeDoc(doc: CanvasDoc): void {
	...
	storeWrite(`canvases/${doc.id}.json`, JSON.stringify(doc));
}
...
// One-time migration from old localStorage keys into ~/.loom.
async function migrateLegacy(): Promise<CanvasDoc | null> {
	if (typeof localStorage === 'undefined') return null;
	// Multi-canvas migration (most common case after a dev session).
	const multiIndex = localStorage.getItem('loom.canvases');
	if (multiIndex) {
		try {
			const list = JSON.parse(multiIndex) as CanvasMeta[];
			if (list.length) {
				const current = localStorage.getItem('loom.current') ?? list[0].id;
				for (const meta of list) {
					const docRaw = localStorage.getItem('loom.doc.' + meta.id);
					if (docRaw) storeWrite(`canvases/${meta.id}.json`, docRaw);
				}
				const settingsRaw = localStorage.getItem('loom.settings');
				if (settingsRaw) storeWrite('settings.json', settingsRaw);
				writeIndex(current, list);
				return null; // index already written; caller reads it
			}
		} catch {}
	}
	// Single-canvas migration (pre-multi-canvas sessions).
	const raw = localStorage.getItem('loom.canvas');
	if (!raw) return null;
	try {
...
export async function init(): Promise<void> {
	let stored = await readIndex();

	if (!stored) {
		const migrated = await migrateLegacy();
		if (migrated) {
			// Single-canvas migration path: migrateLegacy returned the new doc.
			const meta = { id: migrated.id, name: migrated.name, createdAt: migrated.createdAt, updatedAt: migrated.updatedAt };
			library.list = [meta];
			writeIndex(migrated.id, [meta]);
			currentId = migrated.id;
			applyDoc(migrated);
			await loadSettingsAsync();
			return;
		}
		// Multi-canvas migration writes index; re-read it.
		stored = await readIndex();
	}
...
async function loadSettingsAsync(): Promise<void> {
	const raw = await storeRead('settings.json');
