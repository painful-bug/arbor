import type { BBox, Block, BlockType } from "../ast.ts";
	...
	for (let i = start; i < end; i++) {
		...
		if (ocr && nativeChars < PAGE_CHAR_THRESHOLD) {
			...
			for (const reg of readingOrder(detected)) {
				...
				const bbox: BBox = {
					x0: reg.bbox.x0 * pageW,
					y0: reg.bbox.y0 * pageH,
					x1: reg.bbox.x1 * pageW,
					y1: reg.bbox.y1 * pageH,
				};
				const base = {
					page: pageNum,
					readingOrder: 0,
					method: "ocr" as const,
					confidence: conf || reg.confidence,
					bbox,
				};
